...
export default {
  ...
  runScenario: function ( { scenarioId, scenarioState, simState, callbacks } ) {
    ...
    const group = [ 3, 32, 37, 43, 45, 99, 124, 126, 128, 132, 142, 191, 262, 265, 294, 323, 324, 325, 369, 382 ].includes( mappedGroup ) ? mappedGroup : 32;
    ...
    const historicalDataUrl = 'https://storage.googleapis.com/ntd-disease-simulator-data/diseases/trachoma/data/group-45/45-historical-prevalence.csv' //`${groupUrlPath}/${group}-historical-prevalence.csv`;
    const historicalSummaryUrl = 'https://storage.googleapis.com/ntd-disease-simulator-data/diseases/trachoma/data/group-45/45-historical-prevalence-summary.json' //`${groupUrlPath}/${group}-historical-prevalence-summary.json`;
    const scenarioSpecifier = `${group}-${coverage}-${mdaSixMonths}-${mdaRoundsString}`;
    const futureDataUrl = 'https://storage.googleapis.com/ntd-disease-simulator-data/diseases/trachoma/data/group-45/coverage-0.8/mdatype-12/45-0.8-12-202001-202101-202201-202301-202401-202501-202601-202701-202801-202901-203001-prev.csv' //`${mdaUrlPath}/${scenarioSpecifier}-prev.csv`;
    const futureSummaryUrl = 'https://storage.googleapis.com/ntd-disease-simulator-data/diseases/trachoma/data/group-45/coverage-0.8/mdatype-12/45-0.8-12-202001-202101-202201-202301-202401-202501-202601-202701-202801-202901-203001-summary.json' //`${mdaUrlPath}/${scenarioSpecifier}-summary.json`;
    // TODO add -infect.csv

    const historicalDataPromise = new Promise(
      ( resolve, reject ) => {
        console.log( `STHRoundworm loading historical data ${historicalDataUrl}` );
    ...
    const historicalSummaryPromise = new Promise(
      ( resolve, reject ) => {
        console.log( `STHRoundworm loading historical summary ${historicalSummaryUrl}` );
    ...
    const futureDataPromise = new Promise(
      ( resolve, reject ) => {
        console.log( `STHRoundworm loading future data ${futureDataUrl}` );
        csv( futureDataUrl )
        .then( ( results ) => {
          resolve( results );
        } );
    ...
    const futureSummaryPromise = new Promise(
      ( resolve, reject ) => {
        console.log( `STHRoundworm loading future summary ${futureSummaryUrl}` );
